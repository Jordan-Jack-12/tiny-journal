...
const JouranlPageMain = async ({ searchParams }: {
    ...
    const response = await getJournalPageByUserIdServerAction({ userId: userId, page: page });
            ...
            <div className='w-full p-2'>
                <div className='flex ml-3 my-2 justify-between items-center'>
                    ...
                    <div className='flex gap-2'>
                        <Link href={`/journal?page=${previous}`} className='bg-white border border-gray-200 rounded-full'><ChevronLeft /></Link>
                        {page}
                        <Link href={`/journal?page=${next}`} className='bg-white border border-gray-200 rounded-full'><ChevronRight /></Link>
                    </div>
                    <div className='flex gap-2'>
                        <input type="text" placeholder='Search for Journals' className='py-2 px-4 border border-gray-200  bg-white rounded-full' />
